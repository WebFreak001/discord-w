...
struct Message
{
	...
	MessagePartRange byPart() const
	{
		return MessagePartRange(content);
	}
}
...
struct MessagePartRange
{
	...
	void popFront()
	{
		if (index == message.length)
			throw new Exception("Attempted to popFront empty message part range");
	}

	MessagePartRange save()
	{
		return this;
	}
	...
	MessagePart front() @property
	{
		...
		if (s.length > 2 && s[0] == '<' && s[$ - 1] == '>')
		{
			if (s.startsWith("<@!"))
			{
				type = MessagePart.Type.userNickname;
				s = s[3 .. $ - 1];
			}
			else if (s.startsWith("<@&"))
			{
				type = MessagePart.Type.role;
				s = s[3 .. $ - 1];
			}
			else if (s.startsWith("<@"))
			{
				type = MessagePart.Type.user;
				s = s[2 .. $ - 1];
			}
			else if (s.startsWith("<#"))
			{
				type = MessagePart.Type.channel;
				s = s[2 .. $ - 1];
			}
			else if (s.startsWith("<:"))
			{
				type = MessagePart.Type.emoji;
				s = s[2 .. $ - 1];
			}
			else if (s.startsWith("<a:"))
			{
				type = MessagePart.Type.animatedEmoji;
				s = s[3 .. $ - 1];
			}
			else
			{
